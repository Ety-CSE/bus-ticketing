...
if ( ! function_exists('bt_delete'))
{
	function bt_delete($uri){
		return anchor( $uri, '<i class="fa fa-trash-alt"></i>', array('onclick' => "return confirm('You are about to delete a record. This cannot be undone. Are you sure?');"
			));
	}
}
...
if ( ! function_exists('bt_view'))
{
	function bt_view($uri){
		return anchor( $uri, '<i class="fa fa-eye "></i>', array('class' => 'text-center'
			));
	}
}
